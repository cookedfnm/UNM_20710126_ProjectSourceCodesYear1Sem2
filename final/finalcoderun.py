...
# ── Arrow manoeuvre ───────────────────────────────────────────────────────
ARROW_HARD_TURN_TIME = 0.3    # seconds to hard-turn in arrow direction
ARROW_SEARCH_SPEED   = 85     # motor speed while spinning to find the line

# ── Arrow error bias (applied after the manoeuvre) ────────────────────────
TURN_BIAS_PX         = 160     # px bias added to the PID error
ARROW_BIAS_TIMEOUT   = 5.0   # seconds the bias stays active
...
def run_face_recognition(shm_arr, lock, duration=10):
    """
    Runs face recognition using frames from shared memory.
    Assumes the robot is already stationary.
...
# ────────────────────────────────────────────────────────────────────────────
W("my_model_ncnn_model/metadata.yaml", r'''
description: Ultralytics YOLO11n model trained on /content/data.yaml
author: Ultralytics
date: '2026-04-10T08:26:28.993339'
version: 8.4.36
...
# ────────────────────────────────────────────────────────────────────────────
W("my_model_ncnn_model/model_ncnn.py", r'''
import numpy as np
import ncnn
import torch
...
print("  - images/ben.jpeg")
print("  - images/dr_hermawan.jpeg")
print("  - my_model_ncnn_model/__pycache__/model_pnnx.cpython-313.pyc")
print("  - my_model_ncnn_model/model.ncnn.bin")
print("  - my_model_ncnn_model/model.ncnn.param")
