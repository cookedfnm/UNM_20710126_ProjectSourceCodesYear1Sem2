...
# Minimum seconds in STATE_COLOUR before exit bias can trigger.
# Prevents false exits from brief colour detections at the entry junction.
MIN_COLOUR_SECS = 2.0
...
print("  Q or ESC  = emergency stop & quit")
...
try:
    while True:
        found_line, error, active_mode = feedold.get_frame_with_overlay(running)
        ...
        key = feedold.cv2.waitKey(1) & 0xFF
        ...
        if state == STATE_BLACK:
            ...
            # Sample entry direction on first frame only
            ...
            # Only allow exit after minimum dwell time
